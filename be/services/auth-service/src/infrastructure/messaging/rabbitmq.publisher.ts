 ...
 *     làm gián đoạn flow chính (đăng ký user vẫn thành công dù notification fail).
 */
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      this.conn.on('error', (err: Error) => {
        console.error('[RABBITMQ-PUB][AUTH] Connection error:', err.message);
        this.conn = null;
        this.ch = null;
      });

      this.conn.on('close', () => {
        console.warn('[RABBITMQ-PUB][AUTH] Connection closed. Will reconnect on next publish.');
        this.conn = null;
        this.ch = null;
      });

      console.log('[RABBITMQ-PUB][AUTH] Connected to RabbitMQ.');
    } catch (err) {
      this.conn = null;
      this.ch = null;
      throw err;
